...
* [TP2](#tp2--subnetting-et-réseau-client) : Subnetting et réseau client
* [Lexique](#lexique)
...
* notion de [*carte réseau*](#carte-réseau-ou-interface-réseau)
...
* la **[passerelle](#passerelle-ou-gateway) ou gateway** est une machine dans le même réseau que nous, qui est chargée de nous permettre de sortir du réseau. **La passerelle est une machine**, qui porte une IP, comme un de nos PC :)
...
* observer ses [interfaces réseau](#carte-réseau-ou-interface-réseau)
```
# Sur Windows : 
ipconfig /all
...
* **calcul d'adressage**
  ...
  * on peut déduire [l'*adresse de réseau*](#adresse-de-réseau)
  * on peut déduire [l'*adresse de broadcast* ("diffusion")](#adresse-de-diffusion-ou-broadcast-address)
  * et souvent, [l'*adresse de gateway* ("passerelle")](#passerelle-ou-gateway)
...
**1.** `/22` est la *notation [CIDR](#cidr--classless-inter-domain-routing)* d'une IP et de son masque. Une autre façon de le dire est :
...
## TP1.5 : [Subnetting](#subnetting) et réseau client
...
**1.** On calcule l'[*adresse de réseau*](#adresse-de-réseau) pour cette IP, ainsi que l'[*adresse de broadcast*](#adresse-de-diffusion-ou-broadcast-address) (pour la marche à suivre, cf [TP1.3.](#3-exemple-de-manipulation-dip-vue-en-cours)) 
```
                             adresse
        adresse réseau         hôte
   .----------------------..---------.
...
**2.** On peut aussi calculer l'adresse de gateway, qui, souvent, est sur l'adresse juste avant la [gateway](#passerelle-ou-gateway) (mais pas tout le temps).
```
                             adresse
        adresse réseau         hôte
   .----------------------..---------.
...
Bien lire la partie sur les [masques de sous-réseau](#masque-de-sous-réseau).
